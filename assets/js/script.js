const questionAnswer = [
    {
        ...
        {
            question: "In every mainline pokemon game the player receives a device that records information about every pokemon they encounter.  What is this device called?",
            options: {
                a: 'PokePedia',
                b: 'EncyclomMon',
                c: 'Notebook',
  ...
  {
		question: "Which of these moves are no longer learnable in Pokemon Sword and Shield",
		options: {
			a: 'Stored Power',
			b: 'Hidden Power',
			c: 'Ancient Power',
  ...
  {
		question: "What stat does the weather sandstorm raise for rock-type pokemon?",
		options: {
			a: 'Speed',
			b: 'HP',
			c: 'Def',
  ...
  {
		question: "What iconic Follow Me support pokemon did Sejun Park include in his world championship winning team in Pokemon X?",
		options: {
			a: 'Clefairy',
			b: 'Togekiss',
			c: 'Pachirisu',
...
function prompt(){
    ...
    submit.addEventListener('click', function(event){
        ...
        if (index+1 < questionAnswerLen) {
            ...
            scoreForm.setAttribute("style", "display: flex; flex-direction: column; justify-content:center; align-items:center;");
            ...
            saveBtn.setAttribute('style', 'background-color: rgba(62, 200, 255, 0.712); color: white; display: block; width: 80px; height: 40px; border-radius: 20px;');
            saveBtn.addEventListener('click', function() {
                localStorage.setItem('savedScores', JSON.stringify([ initialsInput.value, score ]));
                scoreForm.remove();
                saveBtn.remove();
                restart();
            });
    ...
    function restart() {
        let triviaElements = trivia.children;
            ...
            replay.setAttribute("style","background-color: rgba(62, 200, 255, 0.712); color: white; display: block; width: 80px; height: 40px; border-radius: 20px;");
